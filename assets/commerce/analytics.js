...
window.hcPortfolioEvent=(name,resource)=>{if(!['tool_complete','resource_open'].includes(name)||!['refill-cost','home-reset','chomchom','shark-zu102'].includes(resource))return;g('event',qa?'hc_qa_'+name:name,Object.assign({},common,{resource_id:resource}));};
...
document.addEventListener('click',e=>{
 ...
 if(u.origin===location.origin&&p.includes('/tools/')){const resource=u.pathname==='/us/finds/shark-zu102.html'?'shark-zu102':u.pathname.includes('chomchom')?'chomchom':u.pathname.includes('home-reset')?'home-reset':null;if(resource)window.hcPortfolioEvent('resource_open',resource);}
