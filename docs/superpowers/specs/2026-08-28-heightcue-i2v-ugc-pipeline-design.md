...
`gpt-image-gen-2`는 HeightCue가 보존하는 내부 모델 별칭이다. 2026-08-28 기준 OpenAI 공식 Image API의 실제 제공자 모델 ID는 `gpt-image-2`이므로, 구현은 `gpt-image-gen-2` → `gpt-image-2` 매핑을 명시적으로 기록한다. 호출 결과와 생성 매니페스트에는 내부 별칭과 제공자 모델 ID를 둘 다 남기며, 어느 한쪽을 묵시적으로 바꾸지 않는다.
