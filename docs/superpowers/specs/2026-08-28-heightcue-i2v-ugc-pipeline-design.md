...
`gpt-image-gen-2`는 HeightCue가 보존하는 내부 모델 별칭이다. 2026-08-28 기준 실제 이미지 모델은 `gpt-image-2`이며, Hermes의 `openai-codex` 이미지 제공자가 Codex/ChatGPT OAuth 구독 인증으로 이를 호출한다. 운영 tier는 `gpt-image-2-medium`이다. 구현은 `gpt-image-gen-2` → `openai-codex/gpt-image-2-medium` → `gpt-image-2` 매핑을 명시적으로 기록한다. 호출 결과와 생성 매니페스트에는 내부 별칭, Hermes 제공자/tier, 실제 이미지 모델을 모두 남기며, 별도 `OPENAI_API_KEY`를 요구하거나 어느 한쪽을 묵시적으로 바꾸지 않는다.
