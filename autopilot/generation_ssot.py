...
TASK_DIRECTIVES = {
    ...
    "sales_post": 'Return only JSON: {"candidates":[{"id":"a","text":"..."},{"id":"b","text":"..."}]}. Supply at least two candidates with unique IDs for a compliant sales post grounded only in the resolved product. Each candidate first line must be 70 characters or fewer and ends exactly with #ad. Do not use numbered lists; write compact plain-text fragments. Keep every finished candidate at or below 440 characters. The first line must contain a concrete number, question, or explicit contrast. Include one non-fit line that starts exactly with "Skip if:" and copies only the resolved product exact skip_if value. Every candidate must end exactly in this shape, using the resolved product link field copied verbatim: Full breakdown and current listing: <link> (paid link). Never invent an ingredient, efficacy claim, product fact, comparison, family/use history, or medical outcome absent from source_of_truth. Never use emojis, never use affiliate_link in the post body, and never emit a placeholder such as [HeightCue guide link], [link], or URL_HERE.',
    "value_post": 'Return only this JSON object shape: {"candidates":[{"id":"a","text":"..."},{"id":"b","text":"..."}]}. Supply at least two finished candidates with unique IDs for the requested friction stage. Do not return a single {"text":"..."} object and do not return prose outside the JSON object. Ground every candidate only in the validated friction input. Treat each observation as one reported scene, not a population claim: do not invent numbers, prevalence or frequency words (often, usually, many, always, every), first-person experience, family history, dialogue, or facts absent from resolved_payload. For discovery in both KR and US, write a raw, punchy micro-hook in exactly 4-6 nonblank lines; keep every line short; no essay paragraphs; the final line must be an experience question asking whether the reader has encountered the scene. No biography or product/link in discovery or bridge. No teacher wrap-up, moral, generic advice, follow request, or AI conclusion.',
